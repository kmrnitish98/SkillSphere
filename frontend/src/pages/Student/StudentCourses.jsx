...
import { getMyPayments, getProgress } from '../../services/api';
  ...
  useEffect(() => {
    getMyPayments()
      .then(async (res) => {
         ...
         const enrolled = await Promise.all(payments.map(async (payment) => {
           let progressPercentage = 0;
           try {
             const progRes = await getProgress(payment.course._id);
             if (progRes.data?.data) {
               progressPercentage = progRes.data.data.progressPercentage || 0;
             }
           } catch (err) {
